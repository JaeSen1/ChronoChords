...
export default function SignUp() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post("http://localhost:8080/api/registration/save", {
        username: username,
        email: email,
        password: password,
      });
        ...
        <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square>
          <Box sx={{ my: 8, mx: 10, display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 1 }}>
              <TextField margin="normal" required fullWidth id="user" label="Username" name="user" value={username} onChange={(e) => setUsername(e.target.value)} />
              ...
              />
              <Button type="submit" fullWidth variant="contained" sx={{ mt: 3, mb: 2, backgroundColor: "#EF9F9F", "&:hover": { backgroundColor: "#EF9F9F" } }}>
                Create Account
              </Button>
            </Box>
            <Copyright sx={{ mt: 5 }} />
